...
import org.maraist.fa.{DFA, NFA, PFA}
...
class TestDFA extends AnyFlatSpec with Matchers {
  ...
  def pdaBuilder1: PFABuilder[String, Int] = {
    val builder = new PFABuilder[String, Int]
  ...
  def pdaBuilder2: PFABuilder[String, Int] = {
    val builder = PFA.newBuilder[String, Int]
    builder.addInitialState("A", 1.0)
    builder.addState("B")
    builder.addState("C")
    builder.addFinalState("D", 0.5)
  ...
  "Converting from PDABuilder to PDA" `should` "normalize weights" in {
    ...
    val pfa = builder.result

    assert(pfa.transitionProb("A", 1, "A") === 0.0)
    assert(pfa.transitionProb("A", 1, "B") === 0.4 +- 0.000000001)
    assert(pfa.transitionProb("A", 1, "C") === 0.2 +- 0.000000001)
    assert(pfa.transitionProb("A", 1, "D") === 0.0)

    assert(pfa.transitionProb("A", 2, "A") === 0.0)
    assert(pfa.transitionProb("A", 2, "B") === 0.0)
    assert(pfa.transitionProb("A", 2, "C") === 0.4 +- 0.000000001)
    assert(pfa.transitionProb("A", 2, "D") === 0.0)

    assert(pfa.transitionProb("A", 3, "A") === 0.0)
    assert(pfa.transitionProb("A", 3, "B") === 0.0)
    assert(pfa.transitionProb("A", 3, "C") === 0.0)
    assert(pfa.transitionProb("A", 3, "D") === 0.0)

    assert(pfa.transitionProb("B", 1, "A") === 0.0)
    assert(pfa.transitionProb("B", 1, "B") === 0.0)
    assert(pfa.transitionProb("B", 1, "C") === 0.0)
    assert(pfa.transitionProb("B", 1, "D") === 0.0)

    assert(pfa.transitionProb("B", 2, "A") === 0.0)
    assert(pfa.transitionProb("B", 2, "B") === 0.0)
    assert(pfa.transitionProb("B", 2, "C") === 0.0)
    assert(pfa.transitionProb("B", 2, "D") === 0.0)

    assert(pfa.transitionProb("B", 3, "A") === 0.5 +- 0.000000001)
    assert(pfa.transitionProb("B", 3, "B") === 0.0)
